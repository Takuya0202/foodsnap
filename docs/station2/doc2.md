...
- 上下スライドで次の店舗の切り替え、左右にスライドでメニューの切り替えができるようにする。
...
- モーダルではtopページと同じよう、左右スライドでメニューの切り替え、上下スライドで店舗切り替えが可能
...
- emailとパスワードを使って登録する。
- 店舗位置の登録方法：
  1. **地図でピン差し登録**：地図上をクリック/タップしてピンを配置
  2. **住所入力登録**：テキストフィールドに住所を入力
  3. **現在地登録**：GPSを使用して現在地にピンを設置
- 住所形式：
  - `東京都渋谷区神南1-2-3` 形式
  - `東京都渋谷区神南1丁目2番3号` 形式
  - 都道府県の省略も可能
- MapboxのGeocoding APIを使用：
  - ピン位置から住所を自動取得（リバースジオコーディング）
  - 住所入力から地図上の位置を表示（フォワードジオコーディング）
- 登録後は地図上で店舗位置を確認・修正可能
